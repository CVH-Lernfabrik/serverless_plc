...
# Accept connections from port 4840 (OPC UA)
sudo iptables -A INPUT -p tcp --sport 4840 -j ACCEPT
sudo iptables -A OUTPUT -p tcp --dport 4840 -j ACCEPT

# Accept connections from port 8443 (AWS Greengrass Discovery RESTful API)
sudo iptables -A INPUT -p tcp --sport 8443 -j ACCEPT
sudo iptables -A OUTPUT -p tcp --dport 8443 -j ACCEPT

# Accept connections on port 8883 (MQTTS)
sudo iptables -A INPUT -p tcp --dport 8883 -j ACCEPT
sudo iptables -A OUTPUT -p tcp --sport 8883 -j ACCEPT

# Accept connections from port 8883 (MQTTS)
sudo iptables -A INPUT -p tcp --sport 8883 -j ACCEPT
