...
GGC_SUB_ARN=$(aws --profile ${1} --region ${2} greengrass create-subscription-definition-version --subscription-definition-id ${GGC_SUB_ID} --subscriptions Id="${GGC_SUB_ID}-1",Source="cloud",Subject="opcua_gw/things/+",Target="${OPCUA_GW_ARN}" Id="${GGC_SUB_ID}-2",Source="GGShadowService",Subject="\$aws/things/+/shadow/update/delta",Target="${OPCUA_GW_ARN}" Id="${GGC_SUB_ID}-3",Source="${OPCUA_GW_ARN}",Subject="\$aws/things/+/shadow/#",Target="GGShadowService" | jq -r '.Arn')
