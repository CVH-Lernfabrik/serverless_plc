...
GGC_FNC_ID=$(aws --profile ${1} --region ${2} greengrass list-function-definitions | jq -r '.Definitions[] | select(.Name == "hello-world-template-lambda") | .Id')
if [ -z "${GGC_FNC_ID}" ]; then
    GGC_FNC_ID=$(aws --profile ${1} --region ${2} greengrass create-function-definition --name hello-world-template-lambda | jq -r '.Id')
fi
...
GGC_SUB_ID=$(aws --profile ${1} --region ${2} greengrass list-subscription-definitions | jq -r '.Definitions[] | select(.Name == "hello-world-template-subscription") | .Id')
if [ -z "${GGC_SUB_ID}" ]; then
    GGC_SUB_ID=$(aws --profile ${1} --region ${2} greengrass create-subscription-definition --name hello-world-template-subscription | jq -r '.Id')
fi
GGC_SUB_ARN=$(aws --profile ${1} --region ${2} greengrass create-subscription-definition-version --subscription-definition-id ${GGC_SUB_ID} --subscriptions Id="${GGC_SUB_ID}",Source="${LAMBDA_ARN}",Subject="#",Target="cloud" | jq -r '.Arn')
